...
/obj/machinery/recycling_tank/ComponentInitialize()
...
/obj/machinery/recycling_tank/attackby(obj/item/I, mob/user, params)
	if(I.GetComponent(/datum/component/storage))
		var/cache = content_atoms.len
		for(var/obj/item/item in I.contents)
			insert_atom(I, user)
		if(cache != content_atoms.len)
			turn_on()
		else
...
/obj/machinery/recycling_tank/proc/turn_on()
	...
	if(enabled || !is_operational || storage_full)
		return
...
/obj/machinery/recycling_tank/process()
	...
	for(var/atom/movable/A as anything in content_atoms)
	...
	if(!content_atoms.len && (reagents.total_volume <= reagents.get_reagent_amount(BIOMASS_REAGENT_PATH)))
		turn_off()
