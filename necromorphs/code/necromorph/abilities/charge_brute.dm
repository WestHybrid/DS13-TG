/datum/action/cooldown/necro/charge_brute
	name = "Brute Charge"
...
/datum/action/cooldown/necro/charge_brute/New(Target, delay, past, distance, speed, damage, destroy)
	. = ..()
	if(!isnull(delay))
		charge_delay = delay
	if(!isnull(past))
...
/datum/action/cooldown/necro/charge_brute/Activate(atom/target_atom)
	// start pre-cooldown so that the ability can't come up while the charge is happening
	StartCooldown(10 SECONDS)
	charge_sequence(owner, target_atom, charge_delay, charge_past)
	StartCooldown()

/datum/action/cooldown/necro/charge_brute/proc/charge_sequence(atom/movable/charger, atom/target_atom, delay, past)
	do_charge(owner, target_atom, charge_delay, charge_past)

/datum/action/cooldown/necro/charge_brute/proc/do_charge(atom/movable/charger, atom/target_atom, delay, past)
...
/datum/action/cooldown/necro/charge_brute/proc/pre_move(datum)
	SIGNAL_HANDLER
	// If you sleep in Move() you deserve what's coming to you
	actively_moving = TRUE

/datum/action/cooldown/necro/charge_brute/proc/post_move(datum)
	SIGNAL_HANDLER
	actively_moving = FALSE

/datum/action/cooldown/necro/charge_brute/proc/charge_end(datum/move_loop/source)
	SIGNAL_HANDLER
...
/datum/action/cooldown/necro/charge_brute/proc/stat_changed(mob/source, new_stat, old_stat)
	SIGNAL_HANDLER
	if(new_stat == DEAD)
		SSmove_manager.stop_looping(source) //This will cause the loop to qdel, triggering an end to our charging

/datum/action/cooldown/necro/charge_brute/proc/do_charge_indicator(atom/charger, atom/charge_target)
...
/datum/action/cooldown/necro/charge_brute/proc/on_move(atom/source, atom/new_loc)
	SIGNAL_HANDLER
	if(!actively_moving)
...
/datum/action/cooldown/necro/charge_brute/proc/on_moved(atom/source)
	SIGNAL_HANDLER
	playsound(source, 'sound/effects/meteorimpact.ogg', 200, TRUE, 2, TRUE)
	INVOKE_ASYNC(src, .proc/DestroySurroundings, source)

/datum/action/cooldown/necro/charge_brute/proc/DestroySurroundings(atom/movable/charger)
	if(!destroy_objects)
		return
	if(!isanimal(charger))
		return
...
/datum/action/cooldown/necro/charge_brute/proc/on_bump(atom/movable/source, atom/target)
	SIGNAL_HANDLER
	if(owner == target)
		return
	if(isturf(target))
...
/datum/action/cooldown/necro/charge_brute/proc/hit_target(atom/movable/source, atom/target, damage_dealt)
	if(!isliving(target))
		return
...
/datum/action/cooldown/necro/charge_brute/basic_charge
	name = "Basic Charge"
...
/datum/action/cooldown/necro/charge_brute/basic_charge/do_charge_indicator(atom/charger, atom/charge_target)
	charger.Shake(shake_pixel_shift, shake_pixel_shift, shake_duration)

/datum/action/cooldown/necro/charge_brute/basic_charge/hit_target(atom/movable/source, atom/target, damage_dealt)
...
/datum/action/cooldown/necro/charge_brute/triple_charge
	name = "Triple Charge"
	desc = "Allows you to charge three times at a chosen position."
	charge_delay = 0.6 SECONDS

/datum/action/cooldown/necro/charge_brute/triple_charge/charge_sequence(atom/movable/charger, atom/target_atom, delay, past)
	for(var/i in 0 to 2)
		do_charge(owner, target_atom, charge_delay - 2 * i, charge_past)

/datum/action/cooldown/necro/charge_brute/hallucination_charge
...
/datum/action/cooldown/necro/charge_brute/hallucination_charge/charge_sequence(atom/movable/charger, atom/target_atom, delay, past)
	if(!enraged)
...
/datum/action/cooldown/necro/charge_brute/hallucination_charge/do_charge(atom/movable/charger, atom/target_atom, delay, past)
	. = ..()
	if(charger != owner)
		qdel(charger)

/datum/action/cooldown/necro/charge_brute/hallucination_charge/proc/hallucination_charge(atom/target_atom, clone_amount, delay, past, radius, use_self)
...
/datum/action/cooldown/necro/charge_brute/hallucination_charge/hit_target(atom/movable/source, atom/A, damage_dealt)
	var/applied_damage = charge_damage
	if(source != owner)
		applied_damage = hallucination_damage
	. = ..(source, A, applied_damage)

/datum/action/cooldown/necro/charge_brute/hallucination_charge/hallucination_surround
...
/datum/action/cooldown/necro/charge_brute/hallucination_charge/hallucination_surround/charge_sequence(atom/movable/charger, atom/target_atom, delay, past)
	for(var/i in 0 to 4)
