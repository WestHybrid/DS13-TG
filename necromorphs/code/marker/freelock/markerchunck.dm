...
/datum/markerchunk/proc/add(mob/camera/marker/eye)
...
/datum/markerchunk/proc/remove(mob/camera/marker/eye, remove_static_with_last_chunk = TRUE)
	eye.visibleCameraChunks -= src
	seenby -= eye
 ...
 */
/datum/markerchunk/proc/hasChanged(update_now = 0)
...
/datum/markerchunk/proc/update()
	...
	for(var/atom/current_eye as anything in cameras)
		if(!current_eye || !current_eye.can_use_marker())
			continue

		var/turf/point = locate(src.x + (CHUNK_SIZE / 2), src.y + (CHUNK_SIZE / 2), src.z)
		if(get_dist(point, current_eye) > CHUNK_SIZE + (CHUNK_SIZE / 2))
			continue

		for(var/turf/vis_turf in current_eye.can_see_marker())
			if(turfs[vis_turf])
	...
	for(var/mob/camera/marker/client_eye as anything in seenby)
		var/client/client = client_eye.marker?.client || client_eye.client
		if(!client)
			continue
...
/datum/markerchunk/New(x, y, z)
	...
	for(var/turf in turfs)//one for each 16x16 = 256 turfs this camera chunk encompasses
		inactive_static_images += new/image(GLOB.markernet.obscured)

	for(var/atom/marker_eye as anything in cameras)
		if(!marker_eye)
			continue

		if(!marker_eye?.can_use_marker())
			continue

		for(var/turf/vis_turf in marker_eye?.can_see_marker())
			if(turfs[vis_turf])
