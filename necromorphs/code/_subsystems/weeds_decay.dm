SUBSYSTEM_DEF(weeds_decay)
	...
	var/list/obj/structure/necromorph/weeds/decaying_list = list()
...
/datum/controller/subsystem/weeds_decay/fire(resumed = FALSE)
	for(var/obj/structure/necromorph/weeds/weed as anything in decaying_list)
		if(MC_TICK_CHECK)
			return
		...
		for(var/direction in GLOB.cardinals)
			...
			if(locate(/obj/structure/necromorph/weeds) in adj)
				decay_chance -= rand(15, 20)

		if(prob(decay_chance))
			addtimer(CALLBACK(weed, /obj/structure/necromorph/weeds.proc/check_for_parent_node), rand(3, 10 SECONDS))
			decaying_list -= weed
